...
export function createBrowserClient(): AxiosInstance {
  ...
  client.interceptors.request.use((config) => {
    if (typeof window !== 'undefined') {
      ...
      const token = Cookies.get(process.env.NEXT_PUBLIC_COOKIE_NAME ?? 'hab_constructora_token')
      if (token) config.headers.Authorization = `Bearer ${token}`
      const perfilId = Cookies.get('hab_constructora_perfil_id')
      if (perfilId) config.headers['X-Perfil-Id'] = perfilId
    }
    return config
  })
  ...
  client.interceptors.response.use(
    ...
    (error) => {
      if (error.response?.status === 401 && typeof window !== 'undefined') {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const Cookies = require('js-cookie')
        Cookies.remove(process.env.NEXT_PUBLIC_COOKIE_NAME ?? 'hab_constructora_token')
